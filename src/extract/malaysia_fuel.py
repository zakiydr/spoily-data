...
Key schema notes (from live API testing):
  ...
  - ron95      → non-subsidized retail price (what expats / non-IC holders pay)
  - ron95_budi95 → BUDI 95 subsidized price (for eligible citizens)
  - ron95_skps   → SKPS subsidized price
...
def fetch_fuel_prices(start_date: str, end_date: str) -> pd.DataFrame:
    ...
    Returns:
        DataFrame with columns [date, ron95, ron95_subsidy, ron97]
    ...
    if not data:
        ...
        return pd.DataFrame(columns=["date", "ron95", "ron95_subsidy", "ron97"])
    ...
    result = pd.DataFrame({
        ...
        "ron95_subsidy": pd.to_numeric(df.get("ron95_budi95", pd.Series(dtype="float64")), errors="coerce"),
        "ron97": pd.to_numeric(df["ron97"], errors="coerce"),
    })

    result = result.dropna(subset=["ron95", "ron97"]).sort_values("date").reset_index(drop=True)

    logger.info(f"data.gov.my: final DataFrame has {len(result)} rows, range {result['date'].min()} to {result['date'].max()}")
    return result
...
def _request_with_retry(url: str, params: dict) -> list:
    ...
    for attempt in range(1, HTTP_RETRY_COUNT + 1):
        try:
            ...
            logger.warning(f"data.gov.my request attempt {attempt}/{HTTP_RETRY_COUNT} failed: {exc}. Retrying in {wait}s...")
